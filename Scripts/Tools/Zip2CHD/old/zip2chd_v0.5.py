...
class CHDConverterApp:
    ...
    def handle_original_files(self, cue_file):
        # Mover o eliminar el directorio completo en lugar de solo el archivo .cue
        parent_dir = os.path.dirname(cue_file)
        if self.original_action.get() == "Eliminar":
            shutil.rmtree(parent_dir)
        elif self.original_action.get() == "Mover a 'ORIGINAL'":
            original_dir = os.path.join(self.input_directory.get(), 'ORIGINAL')
            os.makedirs(original_dir, exist_ok=True)
            shutil.move(parent_dir, original_dir)
        # Si es "Mantener", no hacemos nada
    ...
    def cancel_process(self):
        ...
        self.start_button.config(state=tk.NORMAL)
        self.cancel_button.config(state=tk.DISABLED)
        self.log_to_terminal("Proceso cancelado.")

    def exit_program(self):
        if self.processing:
            if not messagebox.askokcancel("Salir", "El proceso está en ejecución. ¿Seguro que quieres salir?"):
                return
        self.master.quit()
        self.master.destroy()
