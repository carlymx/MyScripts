...
def organize_single_file(args):
    ...
    # 2. INTENTO 2: OBTENER FECHA DE MODIFICACIÓN DEL ARCHIVO (FALLBACK)
    if not file_date:
        try:
...
def run_organizer(files_list, dest_dir, workers, logger):
    ...
    processed, errors, exif_count, file_date_count = 0, 0, 0, 0
    ...
    return total, processed, errors, exif_count, file_date_count
...
def main():
    ...
    total, processed, errors, exif, fallback = run_organizer(files_list, dest_dir, n_procs, logger)
