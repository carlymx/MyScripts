#!/usr/bin/env python3
"""
Compresor Fotográfico v3.0 - Añade multiprocesamiento (opcional)
Mantiene la interfaz y estética de la v2.x, pero permite ejecutar el procesamiento
en paralelo usando ProcessPoolExecutor.

Crear entorno virtual automático en Linux si es necesario e instalar dependencias:
- Pillow
- piexif
- colorama

Uso: python3 Compresor_Fotografico_v3.0.py
"""
...
def ensure_dependencies():
    # Forzar venv en Linux para evitar problemas PEP 668
...
def setup_logger(log_file):
    ...
    # Evitar añadir handlers múltiples si se ejecuta varias veces
...
def safe_exif_dump(exif_bytes):
    """
    Recibe los bytes EXIF crudos (o None) y devuelve un objeto exif para pasarlo a Pillow.
    Intentamos reutilizar piexif cuando sea posible; si falla devolvemos None.
    """
    if not exif_bytes:
        return None
    try:
        ...
        # piexif.dump espera dict válido; si falla, devolvemos None
...
def map_tiff_compression(opt_str):
    """Mapea las opciones de compresión al valor esperado por Pillow."""
    if not opt_str:
        return None
    opt = opt_str.lower()
    if opt == 'lzw':
...
def process_single_image(args):
    """
    Función pickeable para ejecutar en procesos hijos.
    args: (file_path, src_dir, dest_dir, size_limit, output_format, compression_opts, quality)
    Devuelve diccionario con resultado para logging por el proceso principal.
    """
    file_path, src_dir, dest_dir, size_limit, output_format, compression_opts, quality = args
    result = {
        'src': file_path,
        'dest': None,
        'ok': False,
    ...
    try:
        ...
        if result['size_kb'] <= size_limit:
            result['ok'] = False
            result['error'] = 'Ignorado por tamaño'  # no es un error real
            return result

        # abrir imagen (PIL). Si no la reconoce, intentamos rawpy si está disponible
        try:
        ...
        if ext_for_format == 'TIFF':
            ...
            save_params['subsampling'] = 0  # evitar chroma subsampling por defecto si queremos calidad
...
def process_images_sequential(files_list, src_dir, dest_dir, size_limit, output_format, compression_opts, quality, logger):
    processed = 0
    large_files = 0
    errors = 0
    ...
    for idx, file_path in enumerate(files_list, start=1):
        res = process_single_image((file_path, src_dir, dest_dir, size_limit, output_format, compression_opts, quality))
...
def process_images_multiprocess(files_list, src_dir, dest_dir, size_limit, output_format, compression_opts, quality, logger, workers):
    total = len(files_list)
    processed = 0
    large_files = 0
    errors = 0
    ...
    tasks = [(fp, src_dir, dest_dir, size_limit, output_format, compression_opts, quality) for fp in files_list]
...
def main():
    ...
    if output_format in ["JPG", "JPEG"]:
        quality_input = get_input("Ingrese nivel de calidad (1-100, 1 = máxima compresión, 100 = mínima compresión) [70]: ", "70")
        try:
    ...
    if n_procs == 1 or len(files_list) <= 1:
        total_files, large_files, processed_files, errors = process_images_sequential(files_list, src_dir, dest_dir, size_limit, output_format, compression_opts, quality, logger)
    else:
