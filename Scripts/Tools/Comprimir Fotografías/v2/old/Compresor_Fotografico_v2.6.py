...
def print_banner():
 ...
 | |__| (_) | | | | | | |_) | (_) | |  | |_| |_| | (_| |  __/ |    
  \____\___/|_| |_| |_| .__/ \___/|_|   \__|\__,_|\__,_|\___|_|    
    ...
    print(Fore.YELLOW + "Versión 2.6 - Compresor Fotográfico")
...
def detect_os():
    ...
    print(Fore.GREEN + f"Sistema operativo detectado: {os_name}" + Style.RESET_ALL)
    return os_name
...
def choose_option(prompt, options, default_index=0):
    ...
    for idx, opt in enumerate(options, start=1):
        print(Fore.CYAN + f"{idx}. {opt}" + Style.RESET_ALL)
...
def process_images(src_dir, dest_dir, size_limit, output_format, compression_opts, quality, logger):
    ...
    for idx, file_path in enumerate(files_list, start=1):
        try:
            ...
            with Image.open(file_path) as img:
                ...
                if safe_exif:
                    img.save(dest_path, format=ext_for_format, exif=safe_exif, **save_params)
                else:
            ...
            errors += 1
        print(Fore.GREEN + f"\rProgreso: {idx}/{total_files} archivos procesados" + Style.RESET_ALL, end="")
...
if __name__ == "__main__":
    ...
    total_files, large_files, processed_files, errors = process_images(src_dir, dest_dir, size_limit, output_format, compression_opts, quality, logger)
