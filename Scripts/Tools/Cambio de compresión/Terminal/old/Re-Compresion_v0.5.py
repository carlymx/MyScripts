...
Correo electrónico: mi_correo@gmail.com
...
3. Pide en qué formato quieres recomprimir los archivos (.zip, .7z, .tar.gz).
...
def compress_files(temp_dir, output_path, format, compression_level):
    import py7zr
    format = format.lstrip('.')  # Remove leading dot
    if format == 'zip':
...
def main():
    ...
    for file in compressed_files:
        ...
        compress_files(temp_dir, output_path, recompression_format.lstrip('.'), compression_level)
