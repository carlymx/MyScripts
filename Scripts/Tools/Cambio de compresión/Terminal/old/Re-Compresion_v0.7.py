...
2. Analiza el directorio y muestra la cantidad de archivos comprimidos que contiene y en qué formatos (.zip, .7z, .tar.gz, .rar).
3. Pide en qué formato quieres recomprimir los archivos (1: .zip, 2: .7z, 3: .tar.gz, 4: .rar).
...
def check_and_install_dependencies():
    ...
    try:
        ...
        print("rarfile no está instalado. Instalando...")
...
def compress_files(temp_dir, output_path, format, compression_level):
    ...
    if format == 'zip':
        ...
        # rarfile library does not support compression, so we need to use the system's rar command line tool
